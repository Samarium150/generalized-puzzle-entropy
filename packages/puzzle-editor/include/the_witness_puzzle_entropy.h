...
class WitnessPuzzleEntropy
    : public Entropy<WitnessState<width, height>, WitnessAction> {
...
public:
    AdversaryEntropyInfo CalculateAdversarialEntropy(
        const SearchEnvironment<WitnessState<width, height>, WitnessAction>
            &env,
        WitnessState<width, height> &state, unsigned lookahead) {
        ...
        if (auto it = std::find_if(actions.cbegin(), actions.cend(),
                                   [&](const auto &action) {
            ...
            children.emplace_back(
                CalculateAdversarialEntropy(env, state, lookahead));
        ...
        H::actCache.returnItem(&actions);
        bool containsSolutionPath =
            std::find_if(children.cbegin(), children.cend(),
                         [](const auto &info) {
                             return info.on_solution_path;
                         }) != children.cend();
        auto &maxChild =
